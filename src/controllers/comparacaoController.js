	...
	for (const [numPedido, pedidoTiny] of mapaTiny) {
		try {
			...
			if (pedidoNuvem) {
				...
				processProdutosPareados(
					pedidoTiny, pedidoNuvem, numPedido, todosResultados
				)
			} else {
				pedidosNaoPareados++
...
const processProdutosPareados = (
	pedidoTiny, pedidoNuvem, numPedido, resultados
) => {
	...
	pedidoNuvem.produtos.forEach((produtoNuvem) => {
		...
		const encontradoNoTiny = pedidoTiny.produtos.some((p) => p.sku === produtoNuvem.sku)
