import { fetchTiktokAuth, fetchTiktokAds, fetchTiktokCreative } from "../services/tiktokServices.js"
...
export const getTikTokCreative = async (req, res) => {
	try {
		const { store, createdAtMin, createdAtMax } = req.params
		const ads = await fetchTiktokCreative(store, createdAtMin, createdAtMax)
		res.status(200).json(ads)
	} catch (error) {
		console.error(error.data)
		const status = error.status
