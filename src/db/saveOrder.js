...
import { logAtacado, logDB, logWebhookMarketplace } from "../utils/logger.js"

export const saveOrder = async (order, tableName) => {
...
export const saveOrderAtacado = async (order) => {
	// Validação básica dos parâmetros
	if (!order) {
		throw new Error("Parâmetro order é obrigatório")
	}

	const tableName = "pedidos_atacado"
	...
	const formattedOrder = {
		...
		id_natureza_operacao: order.id_natureza_operacao || null,
		produtos: JSON.stringify(order.produtos || []),
		produtos_skus: JSON.stringify(order.produtosSkus || []),
		qtd_produtos: order.qtdProdutos || null,
		cpfcnpj_cli: order.cpfcnpjCli || null
	}
	...
	logAtacado(`Pedido ${order.id} salvo com sucesso na tabela ${tableName} | ${order.data_pedido}`)
	return result.rows[0]
}
...
export const updateOrderAtacadoStatus = async (order) => {
	// Validação básica dos parâmetros
	if (!order || !order.id) {
		throw new Error("Parâmetro order e order.id são obrigatórios")
	}
  ...
  `
	...
	if (result.rows.length === 0) {
		logAtacado(`Pedido com ID ${order.id} não encontrado`)
		return {
			success: false,
			message: `Pedido com ID ${order.id} não encontrado`,
			data: null
	...
	logAtacado(`Status do pedido ${order.id} atualizado com sucesso na tabela ${tableName}`)
	return {
		success: true,
		message: "Pedido atualizado com sucesso",
		data: result.rows[0],
