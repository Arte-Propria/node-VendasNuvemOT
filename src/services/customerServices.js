...
export const fetchCustomers = async (params = {}) => {
	...
	if (!code || !storeId) {
		throw new Error("API credentials are not properly set in the environment variables")
	}
	...
	adjustedCreatedAtMax.setUTCHours(
		23, 59, 59, 999
	)
	...
	try {
		...
		while (url) {
			...
			const customers =
        Array.isArray(data) && data.length > 0 ? data.map((customer) => ({	id: customer.id,	name: customer.name,	createdAt: customer.created_at,	buy: customer.last_order_id})): []
