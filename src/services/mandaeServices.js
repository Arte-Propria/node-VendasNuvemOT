...
export const webhookMandaeInfo = async (mandaeData) => {
  try {
    ...
    const entregaRealizada = finalStatusArray.find(
      (event) => event.id === '1' && event.name === 'Entrega realizada',
    );
    ...
    if (entregaRealizada) {
      ...
      situacao = 'OK';
      logMandae(
        `Pedido ${id_ped} marcado como OK - Entrega realizada encontrada`,
      );
    } else {
...
const parseStatusMandae = (statusMandae) => {
  if (!statusMandae) return [];

  try {
    console.log(
      'Tentando parse do status_mandae:',
      typeof statusMandae,
      statusMandae,
    );
    ...
    if (typeof statusMandae === 'string') {
      ...
      try {
        ...
        try {
          ...
          console.log(
            'Segunda tentativa de parse falhou:',
            secondError.message,
          );
...
export const batchUpdateSituacaoFromMandaeEvents = async (batchSize = 100) => {
  try {
    console.log(
      `🚀 Iniciando atualização em lote com tamanho de lote: ${batchSize}...`,
    );
    ...
    while (hasMore) {
      ...
      console.log(
        `📊 Lote ${batchNumber}: ${batchResult.rows.length} pedidos encontrados para análise`,
      );
      ...
      for (const order of batchResult.rows) {
        try {
          ...
          if (entregaRealizada) {
            console.log(
              `✅ ENCONTROU ENTREGA REALIZADA: ${entregaRealizada.name}`,
            );
          ...
          console.error(
            `❌ Erro ao processar pedido ${order.cod_ped}:`,
            error.message,
          );
          failedOrders.push({
    ...
    if (failedOrders.length > 0) {
      ...
      failedOrders.forEach((failed) => {
        console.log(
          `   - Lote ${failed.batch}: ${failed.cod_ped} - ${failed.error}`,
        );
      });
    }
    ...
    console.error(
      '💥 Erro crítico na função batchUpdateSituacaoFromMandaeEvents:',
      error,
    );
    return {
      success: false,
...
export const batchUpdateSituacaoFromMandaeEventsParallel = async (
  batchSize = 100,
  parallelBatches = 3,
) => {
  try {
    console.log(
      `🚀 Iniciando atualização em lote paralela (${parallelBatches} lotes paralelos de ${batchSize})...`,
    );
    ...
    console.log(
      `📦 Serão processados ${totalBatches} lotes de ${batchSize} pedidos`,
    );
    ...
    for (
      ...
      for (let i = 0; i < currentBatches; i++) {
        ...
        batchPromises.push(
          processBatch(currentBatch, offset, batchSize).then((result) => {
            ...
            console.log(
              `✅ Lote ${currentBatch} concluído: ${result.updated}/${result.processed} atualizados`,
            );
      ...
      if (batchIndex + parallelBatches < totalBatches) {
        console.log(
          '⏳ Aguardando 2 segundos antes do próximo grupo de lotes...',
        );
...
export const debugParseStatusMandaeByCodPed = async (codPed) => {
  ...
  try {
    ...
    console.log('Executando query:', findOrderQuery, 'com parâmetro:', codPed);

    const orderResult = await query(findOrderQuery, [codPed]);

    if (orderResult.rows.length === 0) {
      throw new Error(
        `Pedido com cod_ped "${codPed}" não encontrado na base de dados`,
      );
    }
    ...
    if (entregaRealizada && situacao === 'NOK') {
      ...
      console.log(
        'ℹ️  Pedido com situação = NOK mas sem evento de entrega realizada (nada a fazer)',
      );
    }
