...
export const filterBdByDateRange = (
  queryData,
  querySelect,
  { startDate, endDate } = {},
) => {
  try {
    ...
    if (!dateField) {
      console.warn(
        `⚠️ Tabela '${querySelect}' não possui campo de data mapeado. Retornando dados sem filtro.`,
      );
      return queryData;
    }
    ...
    if (!sampleItem || !sampleItem.hasOwnProperty(dateField)) {
      console.warn(
        `⚠️ Campo '${dateField}' não encontrado nos dados. Campos disponíveis:`,
    ...
    const filteredData = queryData.filter(item => {
      try {
        ...
        if (!itemDateValue) {
          console.warn(
            `⚠️ Item sem data no campo '${dateField}':`,
            item.id || item,
          );
...
*/
  ...
  if (firstOrder) {
    ...
    if (orderDate && storeId) {
      ...
      if (!storeName) {
        console.warn(
          `Store ${storeId} não mapeado. Anúncios não serão vinculados.`,
        );
      }
...
export async function processOrderFromTiny(tinyResponse) {
  ...
  if (!nomeEcommerce) {
    console.error(
      "[processOrderFromTiny] Campo 'pedido.ecommerce.nomeEcommerce' não encontrado no webhook",
    );
  ...
  console.log(
    '[processOrderFromTiny] Nota fiscal obtida:',
  ...
  if (!firstOrder) {
    throw new Error(
      '[processOrderFromTiny] Nenhum pedido encontrado no payload',
    );
  }
  ...
  console.log(
    `[processOrderFromTiny] Processando pedido ID: ${firstOrder.order_id}`,
  );
  ...
  console.log(
    `[processOrderFromTiny] Loja mapeada: ${storeNameTiny} -> ${storeNumeric}`,
  );
  ...
  console.log(
    `[processOrderFromTiny] Pedido ${firstOrder.order_id} salvo/atualizado`,
  );
  ...
  if (storeNameForAds) {
    ...
    adsIds = adsResult.rows.map(row => row.id_ads);
    console.log(
      `[processOrderFromTiny] ${adsIds.length} anúncios encontrados para ${orderDate}`,
    );
  }
  ...
  console.log(
    `[processOrderFromTiny] Daily sales atualizado para data ${orderDate}, loja ${storeNumeric}`,
  );
